...
TypeId Bgp::GetTypeId (void) {
    static TypeId tid = TypeId("ns3::Bgp")
        ...
        .AddConstructor<Bgp>()
        .AddAttribute("LibbgpLogLevel", "Log level for libbgp (0=FATAL..4=DEBUG)",
            UintegerValue(static_cast<uint32_t>(libbgp::INFO)),
            MakeUintegerAccessor(&Bgp::_log_level_uint),
...
Bgp::Bgp() : _logger("(local)"), _rib(&_logger) {
    ...
    _log_level_uint = static_cast<uint32_t>(libbgp::INFO);
...
void Bgp::StartApplication(void) {
    ...
    _log_level = static_cast<libbgp::LogLevel>(_log_level_uint);
    _logger.setLogLevel(_log_level);
